...
#==============================================================================
            ...
            elif msg["action"] == "client_request_rsa_key":
                from_name = self.logged_sock2name[from_sock]
                to_name=msg["to_name"]
                rsa_key=self.client_rsa_key[to_name]
                #Server transmit the target receiver's public rsa key to the sender
                msg1=json.dumps({"action": "send_target_rsa_key", "from":from_name,"rsa_key":rsa_key})
                mysend(from_sock, msg1)

            elif msg["action"] == "client_send_rsa_key":
                from_name=self.logged_sock2name[from_sock]
                self.client_rsa_key[from_name]=msg["client_rsa_key"] 
    
            elif msg["action"] == "send_secure_message":
                to_name=msg["to_name"]
                from_name=self.logged_sock2name[from_sock]
                to_sock=self.logged_name2sock[to_name]
                aes_key=msg["encrypted_aes_key"]
                encrypted_msg=msg["encrypted_msg"]
                mysend(to_sock, json.dumps({"action":"receive_secure_message", "from":from_name,
                                            "encrypted_aes_key": aes_key, "message": encrypted_msg}))
#==============================================================================
